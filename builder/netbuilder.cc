...
class NetBuilder : public cSimpleModule
{
  protected:
    double channelDelay;
    double channelBitErrorRate;    //  ratio between the number of bits incorrectly received and the total number of bits transmitted
    double channelDatarate;
...
void NetBuilder::initialize()
{
    ...
    channelBitErrorRate = par("channelBitErrorRate");
    channelDatarate = par("channelDatarate");
}
...
void NetBuilder::buildNetwork(cModule *parent)
{
    ...
    while (getline(nodesFile, line, '\n')) {
        ...
        std::vector<std::string> tokens = cStringTokenizer(line.c_str(), /*delimiters=*/", ").asVector();
        if (tokens.size() != 1)
            throw cRuntimeError("wrong line in module file: 1 node id required, line: \"%s\"", line.c_str());

        // get fields from tokens
        long nodeid = atol(tokens[0].c_str());
        char name[15] = "node";
        strcat(name, std::to_string(nodeid).c_str());
        const char *modtypename = par("nodeType").stringValue();
    ...
    while (getline(connectionsFile, line, '\n')) {
        ...
        std::vector<std::string> tokens = cStringTokenizer(line.c_str(), /*delimiters=*/", ").asVector();
        if (tokens.size() != 2)
            throw cRuntimeError("wrong line in parameters file: 2 node id required, line: \"%s\"", line.c_str());
...
//        double datarate = tokens[4] != "-" ? atof(tokens[4].c_str()) : -1;
        ...
        double error = channelBitErrorRate;
        double datarate = channelDatarate;
